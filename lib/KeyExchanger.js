...
var SecureRandom = bitcore.crypto.Random;
...
function KeyExchanger(transport) {
  ...
  this.key = new PrivateKey(); // eckey for exchange of session key
...
KeyExchanger.prototype.clientHandshake = function(callback) {
  var self = this;
  self.transport.write(shastring(MAGIC_BUF));
  self.transport.write(shastring(self.key.publicKey.toBuffer()));
  self.expectServerPubkey(function(err, pubkey) {
    if (err) return callback(err);
    self.expectPreMasterKey(function(err, preMasterKeyEncrypted) {
      var cypher = new ECIES()
        .privateKey(self.key)
        .publicKey(new PublicKey(pubkey));
      if (err) return callback(err);
      var preMasterKey = cypher.decrypt(preMasterKeyEncrypted);
...
KeyExchanger.prototype.serverHandshake = function(callback) {
  ...
  self.expectMagic(function(err) {
    ...
    self.expectClientPubkey(function(err, pubkey) {
      if (err) return callback(err);
      var cypher = new ECIES().privateKey(self.key).publicKey(new PublicKey(pubkey));
      var myPubKeyEncrypted = cypher.encrypt(self.key.toBuffer());
      self.transport.write(shastring(myPubKeyEncrypted));
      var preMasterKey = SecureRandom.getRandomBuffer(32);
      var preMasterKeyEncrypted = cypher.encrypt(preMasterKey);
...
KeyExchanger.prototype.expectServerPubkey = function(callback) {
  ...
  this.expect(STATE_WAIT_PUBKEY, function(err, message) {
    if (err) return callback(err);
    // TODO: what public key?
    var cypher = new ECIES().privateKey(self.key);
    var serverPubKey = cypher.decrypt(message);
