...
var sha256ripe160 = bitcore.crypto.Hash.sha256ripe160;
...
messages.CONNECT = function(router, peer, message, connection) {
  if (!connection) {
    connection = new Connection(message.connectionId);
...
messages.CONNECTINFO = function(router, peer, message) {
  ...
  for (var i = 0; i < router.endpoints.length; i++) {
    ...
    if (svr.isMatch(pubkey)) {
      ...
      if (!sock.request) {
        request = new ConnectionRequest(connectionId);
      ...
      var connectionIdEncrypted = ECIES.encrypt(svr.key.private, connectionId);
      ...
      peer.CONNECTACK(connectionId, connectionIdEncrypted, preMasterKeyEncrypted);
      return;
